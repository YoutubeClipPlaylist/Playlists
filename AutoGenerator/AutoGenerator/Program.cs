...
foreach (Channel channel in option.Channels)
{
    ...
    WriteJsoncFile(channel, exclusiveWords, newPlaylist);
...
static void WriteJsoncFile(Channel channel, HashSet<string> exclusiveWords, List<ISong> newPlaylist)
{
    ...
    var arr = newPlaylist.Where(p => !exclusiveWords.Contains(p.VideoId)
                                     && !exclusiveWords.Contains(p.Title))
                         .ToArray();

    sw.Write(JsonSerializer.Serialize(arr,
                                      options: new()
                                      {
                                          Encoder = JavaScriptEncoder.Create(UnicodeRanges.All),
                                          WriteIndented = true,
    ...
    sw.Flush();
    Console.WriteLine($"Write {arr.Length} songs to jsonc file.");
